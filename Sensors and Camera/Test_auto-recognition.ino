...
void loop() {
  ...
  if (!learningAttempted && !huskylens.isLearned() && (millis() - startTime >= AUTO_LEARN_DELAY)) {
  ...
  if (!learningAttempted && !huskylens.isLearned()) {
  ...
  if (!huskylens.request()) {
    ...
    // Only show this message if we've already attempted learning
    if (learningAttempted) {
      Serial.println(F("No object has been learned. Try repositioning the object."));
    }
  }
