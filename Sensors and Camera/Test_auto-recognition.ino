...
void setup() {
  ...
  if (retryCount >= 10) {
    ...
    // Automatically learn the object without button press
    if (!huskylens.learnObject()) {
      Serial.println(F("Object training failed. Please make sure an object is in front of the camera."));
    } else {
      Serial.println(F("Object successfully trained."));
      
    }

    delay(2000);  // Allow time for training
    Serial.println(F("Waiting 30 seconds before starting object recognition..."));
    delay(30000);  // 30 seconds before starting the recognition loop
  }
}
