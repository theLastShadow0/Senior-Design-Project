...
#define buzzerPin A4 // Buzzer pin - change this to the appropriate analog pin
...
unsigned long startTime; // To track when setup completed
unsigned long lastDetectionTime = 0; // To track when target was last detected
unsigned long targetLostTime = 0; // To track when target was lost
bool learningAttempted = false; // Flag to track if learning has been attempted
bool targetLost = false; // Flag to track if target is lost
bool buzzerActive = false; // Flag to track buzzer state
bool waitingForTimeout = false; // Flag to track if we're waiting for the 5-second timeout
...
void setup() {
  ...
  if (retryCount >= 10) {
    ...
    // Forget any previously learned objects
    Serial.println(F("Forgetting any previously learned objects..."));
...
void loop() {
  ...
  if (!learningAttempted && (currentTime - startTime >= AUTO_LEARN_DELAY)) {
    ...
    if (huskylens.writeLearn(1)) {
      ...
      learningAttempted = true; // Mark learning as attempted
      ...
      // Reset the start time to try learning again in 5 seconds
  ...
  if (!huskylens.request()) {
    ...
    // If we've attempted learning but nothing is learned, try again
    if (learningAttempted) {
      Serial.println(F("No object has been learned. Trying again..."));
      learningAttempted = false; // Reset so we can try again
      startTime = currentTime - (AUTO_LEARN_DELAY - 5000); // Try again in 5 seconds
    }
  }
  else if (!huskylens.available()) {
    Serial.println(F("No object detected on screen."));
    
    // Stop the car when object is lost
    ...
    // Check if target has been lost for more than the timeout period
    ...
    // If we were waiting for timeout, cancel it
    ...
    // If target was lost and now found, stop the buzzer
    if (targetLost) {
      targetLost = false;
      if (buzzerActive) {
