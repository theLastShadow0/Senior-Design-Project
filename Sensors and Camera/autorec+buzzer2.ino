...
const unsigned long TARGET_LOST_TIMEOUT = 15000; // 15 seconds timeout for target loss
  ...
  if (!huskylens.request()) {
    ...
    if (lastDetectionTime > 0) {
      ...
      // After 15 seconds of beeping, stop and try to relearn
        ...
        // Directly attempt to learn a new object
        Serial.println(F("Auto-learning now..."));
        if (huskylens.writeLearn(1)) {
          Serial.println(F("Object re-learned successfully as ID 1!"));
        } else {
          Serial.println(F("Re-learning failed. Make sure object is visible to the camera."));
          // Reset flags to try again on next loop
